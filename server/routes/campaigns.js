...
router.post('/', auth, async (req, res) => {
  const { name, desc, level, status } = req.body;
  if (!name || !desc) return res.status(400).json({ error: 'Название и описание обязательны.' });
  ...
  try {
    ...
    const { rows } = await pool.query(`
      INSERT INTO campaigns(dm_id, name, "desc", level, status)
      VALUES($1,$2,$3,$4,$5)
      RETURNING id
    `, [req.session.userId, name, desc, level || '1-5', status || 'active']);
